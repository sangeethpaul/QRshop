    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-8 text-center">
        <div className="absolute top-0 left-0 w-full h-full overflow-hidden -z-10 pointer-events-none">
          <div className="absolute top-[10%] left-[20%] w-[40rem] h-[40rem] bg-primary/20 rounded-full blur-[120px] animate-pulse"></div>
          <div className="absolute bottom-[10%] right-[20%] w-[35rem] h-[35rem] bg-secondary/20 rounded-full blur-[120px] animate-pulse"></div>
        </div>
          ...
          {[
            ...
            <button
              ...
              className={`group flex flex-col items-center p-8 rounded-3xl transition-all duration-500 text-center glass glass-hover relative overflow-hidden ${
                plan.highlight ? 'ring-2 ring-primary/50 scale-105 z-10' : 'scale-100'
              }`}
            >
              {plan.highlight && (
              ...
              <p className="text-3xl font-black mb-1 text-white">{plan.price}</p>
              <p className="text-zinc-500 text-sm mb-8 font-medium">{plan.sub}</p>
        ...
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-zinc-500 text-sm">Join 1,000+ creators and businesses</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-12 glass p-6 rounded-3xl">
          <div>
            ...
            <p className="text-zinc-400 font-medium">Welcome back, {session.user?.name || session.user?.email}</p>
          </div>
          <button 
            onClick={() => signOut()} 
            className="px-6 py-2 rounded-xl bg-white/5 hover:bg-white/10 text-zinc-400 hover:text-white transition font-bold text-sm"
          >
            Sign Out
          </button>
        </div>
        ...
        <div className="glass p-8 rounded-3xl mb-12 relative overflow-hidden">
          <div className="absolute -top-24 -right-24 w-48 h-48 bg-primary/20 blur-[80px]"></div>
          <h2 className="text-2xl font-bold mb-6 text-white">Generate QR Code</h2>
          <form onSubmit={handleGenerate} className="flex flex-col md:flex-row gap-4 relative">
            <div className="flex-1 relative group">
              <input
                ...
                className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 pl-12 text-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-all"
              />
              <svg className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-zinc-500 group-focus-within:text-primary transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <button
              type="submit"
              disabled={loading}
              className="px-10 py-4 rounded-2xl font-bold btn-primary disabled:opacity-50 text-lg"
            >
              {loading ? "Generating..." : "Generate Free"}
            </button>
          </form>
          {error && <p className="text-red-400 mt-4 font-medium flex items-center gap-2">
        ...
        <div className="flex items-center justify-between mb-8">
          <h2 className="text-2xl font-bold text-white flex items-center gap-3">
            Your QR Codes
            <span className="bg-white/10 text-zinc-400 px-3 py-1 rounded-full text-sm">{qrCodes.length}</span>
          </h2>
        </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            return (
              <div key={qr.id} className="glass p-6 rounded-3xl flex flex-col items-center text-center group hover:border-primary/50 transition-all duration-300">
                ...
                <div className="mb-6 w-full px-2">
                  <p className="text-xs font-bold text-zinc-500 uppercase tracking-widest mb-2">Destination</p>
                  {editingId === qr.id ? (
                    <div className="flex flex-col gap-2">
                      <input
                        className="w-full bg-white/5 border border-white/20 rounded-xl px-4 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-primary"
                        value={editUrl}
                      ...
                      <div className="flex gap-2">
                        ...
                        <button onClick={() => setEditingId(null)} className="flex-1 bg-white/5 text-zinc-400 py-2 rounded-xl text-xs font-bold">Cancel</button>
                      </div>
                    </div>
                  ) : (
                    <a href={qr.destinationUrl} target="_blank" rel="noreferrer" className="text-blue-400 hover:text-blue-300 transition-colors text-sm font-bold break-all line-clamp-1">
                      {qr.destinationUrl}
                    </a>
                  )}
                </div>

                <div className="flex justify-between w-full text-xs font-bold mb-6 text-zinc-400 bg-white/5 p-4 rounded-2xl">
                  <div className="flex flex-col items-start">
                    <span className="text-[10px] uppercase text-zinc-600 mb-1">Engagements</span>
                    <span className="text-white text-base">{qr.clicks}</span>
                  </div>
                  <div className="flex flex-col items-end">
                    <span className="text-[10px] uppercase text-zinc-600 mb-1">Status</span>
                ...
                <div className="mt-auto w-full space-y-3">
                  ...
                  {qr.isDynamic && editingId !== qr.id && (
                    <button
                      ...
                      className="w-full py-3 rounded-2xl bg-white/5 border border-white/10 text-sm font-bold text-white hover:bg-white/10 transition-all hover:scale-[1.02]"
                    >
                      Update Destination
                    </button>
                  )}
          ...
          {qrCodes.length === 0 && (
            <div className="col-span-full text-center py-24 glass rounded-3xl border-dashed border-2 border-white/10">
              <div className="text-5xl mb-4">✨</div>
              <h3 className="text-xl font-bold text-white mb-2">No QR Codes Yet</h3>
              <p className="text-zinc-500">Generate your first premium QR code above!</p>
            </div>
          )}
        </div>
      </div>
